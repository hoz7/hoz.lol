<script lang="ts">
    ...
    import { UpcomingMovies } from "$lib/UpcomingMovies";
    ...
    onMount(async () => {
        upcomingMovies = await UpcomingMovies(1);
        loading = false;
    });

    const incrementPage = async () => {
        pageNum += 1;
        upcomingMovies = await UpcomingMovies(pageNum);
    };
...
{:else}
    <div class="py-10 px-5">
        <div class="flex items-center justify-between mb-5">
            <h1 class="text-3xl font-semibold">Upcoming</h1>
